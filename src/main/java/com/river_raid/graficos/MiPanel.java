...
import com.river_raid.enemigos.Barco;
import com.river_raid.enemigos.Enemigos;
import com.river_raid.enemigos.Helicoptero;
import com.river_raid.global.CargarImagen;
import com.river_raid.global.Personajes;
import com.river_raid.global.Sonido;
...
import com.river_raid.jugador.Bala;
import com.river_raid.jugador.Combustible;
import com.river_raid.jugador.Jugador;
...
public final class MiPanel extends JPanel {
    ...
    /**
     * Constructor parametrico el cual inicia el menu principal y el menu de pausa
     * asi como las otras variables a usar
     * 
    ...
    public MiPanel(int ancho, int alto) {
        ...
        gun = new Sonido("Disparo.mp3");

        explosion = new Sonido("Explosion.mp3");

        recarga = new Sonido("Recarga.mp3");

        fondo = new Sonido("Fondo.mp3");
     ...
     * @param pausa indica si se sale del juego cuando haya estado en el menu de
     *              pausa
     */
    public void salir_juego(boolean gano, boolean pausa) {
        ...
        if (!pausa) {
            ...
            if (gano) {
                ...
                this.gano = gano;
                JOptionPane.showMessageDialog(null, "                            GANO", "ESTADO DE JUEGO",
                        JOptionPane.INFORMATION_MESSAGE,
                        new ImageIcon(CargarImagen.loadImage("entorno/YouWin.jpg")));
            } else {
                this.perdio = !gano;
                JOptionPane.showMessageDialog(null, "                            PERDIO", "ESTADO DE JUEGO",
                        JOptionPane.INFORMATION_MESSAGE,
                        new ImageIcon(CargarImagen.loadImage("entorno/YouWin.jpg")));
            }
    ...
    /**
     * Metodo que crea aleatoriamente un enemigo con su respectiva orientacion
    ...
    /**
     * Metodo que devuelve una posicion correcta, alejada del objeto mandado por
     * parametro
     * 
     * @param objeto
     * @param NumObjeto
    ...
    public void aparecer(Objeto objeto, int NumObjeto, boolean inicio) {
        ...
        if (inicio) // AL COMIENZO VOY A GENERAR NUMEROS ALEATORIOS DESDE UNA IMAGEN ATRAS, PARA QUE
                    // NO SE VISUALIZEN HASTA MAS DE LA MITAD DE LA VENTANA
        {
            maxy = 2 * ALTO_IMAGEN;
            DELTA = (int) (Math.random() * 300 + 50);
        } else // LUEGO CUANDO LOS ELIMINEN O YA HAYA BAJADO, LOS POSICIONA DESDE MUY ATRAS
               // HASTA EL BORDE QUE ES CERO PARA QUE EL USUARIO NO LOS LLEGASE A VER
        {
            maxy = 0;
    ...
    public void pintarJuego(Graphics2D g2) {
        ...
        g2.drawImage(mapa.getmedidor_combustible(), (int) (162 + 2.67 * jugador.getcombustible()), ALTO_VENTANA - 102,
                this);

        for (int x = 0; x < vidas_jugador; x++)
            g2.drawImage(CargarImagen.loadImage("entorno/Vida.jpg"), 30 + 30 * x,
                    ALTO_VENTANA - 100, null);
        ...
        if (gano) {
            g2.drawImage(CargarImagen.loadImage("entorno/YouWin.png"), 0, 0, ANCHO_VENTANA,
                    ALTO_VENTANA, null);
        }
        if (perdio) {
            g2.drawImage(CargarImagen.loadImage("entorno/GameOver.jpg"), 0, 0, ANCHO_VENTANA,
                    ALTO_VENTANA, null);
        }
        if (disparo) {
    ...
    /**
     * Clase timer jugador encargado de procesar los movimientos y acciones del
     * jugador
     */
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (Enemigos enemigo : enemigos) {
                ...
                if (enemigo instanceof Helicoptero) {
                    enemigo.moverx(velocidad_helicoptero, mapa.getBORDEIZQUIERDO(enemigo.getposy()),
                            mapa.getBORDEDERECHO(enemigo.getposy()));
                } else {
                    enemigo.moverx(velocidad_barco, mapa.getBORDEIZQUIERDO(enemigo.getposy()),
                            mapa.getBORDEDERECHO(enemigo.getposy()));
                }
            }
    ...
    class TimerBala implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (Enemigos enemigo : enemigos) {
                if (bala.getvivo()
                        && (bala.area().intersects(enemigo.area()) || bala.area().contains(enemigo.area()))) {
                    puntuacion += 100;
                    bala.setvivo(false);
                    disparo = false;
            ...
            for (Combustible combustible : combustibles) {
                if (bala.area().intersects(combustible.area()) && bala.getvivo()
                        || bala.area().contains(combustible.area())) {
                    if (puntuacion > 0)
                        puntuacion -= 100;
                    if (puntuacion < 0)
                        puntuacion = 0;
    ...
    /**
     * Clase timer enemigos encargado de procesar los movimientos y acciones de los
     * enemigos
     */
    class TimerEnemigos implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (Enemigos enemigo : enemigos) {
                ...
                if (enemigo.getvivo() && jugador.getvivo()
                        && (enemigo.area().intersects(jugador.area()) || enemigo.area().contains(jugador.area()))) {
    ...
    /**
     * Clase timer combustible encargado de procesar los movimientos y acciones del
     * combustible
     */
    class TimerCombustible implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (int x = 0; x < combustibles.length; x++) {
                ...
                if (combustibles[x].area().intersects(jugador.area())
                        || combustibles[x].area().contains(jugador.area())) {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (velocidad_mapa != 0) {
                if (teclas[KeyEvent.VK_LEFT]) {
                    ...
                    if (jugador.getvivo() && !jugador.moverme(true, false, mapa.getBORDEIZQUIERDO(jugador.getposy()),
                            mapa.getBORDEDERECHO(jugador.getposy()))) {
                        explosion.reproducir();
                ...
                if (teclas[KeyEvent.VK_RIGHT]) {
                    ...
                    if (jugador.getvivo() && !jugador.moverme(false, true, mapa.getBORDEIZQUIERDO(jugador.getposy()),
                            mapa.getBORDEDERECHO(jugador.getposy()))) {
                        explosion.reproducir();
                ...
                if (teclas[KeyEvent.VK_SPACE]) {
                    ...
                    if (bala == null || bala.getposy() < 0 - bala.getimagen().getHeight(null) || !bala.getvivo()) {
                        ...
                        bala = new Bala();
                        bala.setposx(jugador.getposx() + jugador.getimagen().getWidth(null) / 2
                                - bala.getimagen().getWidth(null) / 2);
            ...
            if (!teclas[KeyEvent.VK_DOWN] && !teclas[KeyEvent.VK_UP] && velocidad_mapa != 0) // SI YA NO TENGO
                                                                                             // PRESIONADA LA TECLA
                                                                                             // HACIA ABAJO NI LA TECLA
                                                                                             // HACIA ARRIBA
            {
                velocidad_mapa = velocidad_inicial_mapa;
            }
            if (!teclas[KeyEvent.VK_LEFT] && !teclas[KeyEvent.VK_RIGHT]) // SI YA NO TENGO PRESIONADA NI LA TECLA
                                                                         // IZQUIERDA NI LA DERECHA
            {
                if (jugador.getvivo() && !jugador.sinmover(mapa.getBORDEIZQUIERDO(jugador.getposy()),
                        mapa.getBORDEDERECHO(jugador.getposy()))) {
                    explosion.reproducir();
