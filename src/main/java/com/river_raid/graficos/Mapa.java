...
public final class Mapa extends Objeto {
    ...
    /**
     * Constructor parametrico de la clase mapa, crea el mapa, lo posiciona, carga
     * las imagenes
     * 
    ...
    public Mapa(int ancho, int alto) {
        ...
        posy = 0 - ALTO_IMAGEN; // POSICIONO AL MAPA UNA POSICION ARRIBA DE LA IMAGEN PARA QUE EL USUARIO NO LA
                                // VEA

        mapa = new BufferedImage(ANCHO_MARCO, ALTO_MARCO + ALTO_IMAGEN, BufferedImage.TYPE_INT_RGB);

        estado_combustible = CargarImagen.loadImage("entorno/Estado Combustible.jpg"); // CAMBIAR

        medidor_combustible = CargarImagen.loadImage("entorno/Medidor Combustible.jpg");

        mapas = new BufferedImage[NUM_MAPS];

        for (int x = 0; x < mapas.length; x++)
            mapas[x] = CargarImagen.loadImage("ambiente/Mapa" + x + ".jpg");
    ...
    /**
     * Metodo para generar uno de los mapas a partir del 1 hasta el num mapas
     * establecido
     * 
    ...
    /**
     * Metodo que devuelve un vector de enteros que representan cada color de pixel
     * de la imagen
     * 
    ...
    public void crearMapa() {
        ...
        posy = 0 - ALTO_IMAGEN; // SUBO EL MAPA DE NUEVO PARA QUE NO VEA LA IMAGEN QUE SE VA CREAR A
                                // CONTINUACION
